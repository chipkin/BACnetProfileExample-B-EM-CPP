...
| SubscribeCOVPropertyMultiple client shows no obvious way to test | Neither `bacpypes3` nor `BAC0` implement the SubscribeCOVPropertyMultiple APDU as of this writing (`bacpypes3/apdu.py` marks it `###TODO`). Use the CAS BACnet Explorer, or a client library with the service implemented. |
